...
test-kafka:
	...
	docker compose -f test/compose-kafka.yml up --quiet-pull -d --wait
	go test $(GOTEST_FLAGS) -race ./...; ret=$$?; \
		docker compose -f test/compose-kafka.yml down; \
		exit $$ret

.PHONY: test-redpanda
test-redpanda:
	# run required docker containers, execute integration tests, stop containers after tests
	docker compose -f test/compose-redpanda.yml up --quiet-pull -d --wait
	go test $(GOTEST_FLAGS) -race ./...; ret=$$?; \
		docker compose -f test/compose-redpanda.yml down; \
		exit $$ret
